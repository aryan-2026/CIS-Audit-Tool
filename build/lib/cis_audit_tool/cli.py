...
def main():
    ...
    if sys.platform == "linux":
        ...
        report_dir = "windows"
        out_json = f"{report_dir}/audit-report.json"
        ...
        try:
            subprocess.run([
                ...
                "-Command",
                f"Import-Module .\\windows\\modules\\CISChecks.psm1 -Force; "
                f"$results = .\\windows\\runner.ps1 -Profile 'cis-windows-11.json'; "
            ], check=True)

            logger.info("Windows CIS benchmark audit completed successfully.")

            # Read JSON results using utf-8-sig to handle BOM
